...
> **产品文档**：[`docs/MOBA_BUILD_AGENT_PRD.md`](docs/MOBA_BUILD_AGENT_PRD.md) · **案例研究**：[`docs/CASE_STUDY.md`](docs/CASE_STUDY.md) · **装备与克制关系拆解**：[`docs/GAME_BREAKDOWN.md`](docs/GAME_BREAKDOWN.md)
>
> 想知道"这套建议是怎么算出来的"，直接看拆解小样：它把**装备标签体系（26 类）→ 约束模型（6 格 / 鞋类唯一组 / 复活甲限次）→ 英雄威胁标签（9 类 / 77 个英雄）→ 阵容威胁映射规则（5 条）** 逐层摊开，并用三个真实对局场景做了完整复盘。
...
- **装备知识库**：接入官方 item.json（121 件装备，含来源 URL 与内容哈希校验），提供目录与单件查询，每条装备带价格、功能标签、唯一组、复活甲每局限次、主动冷却等元数据。其中 **28 件核心装备经人工标注功能标签**、另有 20 件由官方描述文本解析补齐，合计 48 件进入 **26 类**标签体系（拆解详见 [`docs/GAME_BREAKDOWN.md`](docs/GAME_BREAKDOWN.md)）。
- **紧急秒换建议**：POST /api/decisions 新增 emergency_swap 段，覆盖辉月(金身)/贤者的庇护(复活甲)/名刀·司命/血魔之怒/纯净苍穹与冷静之靴→抵抗之靴，受金币、格子、鞋类唯一组与复活甲 2 次/局约束。
- **英雄威胁画像**：接入官方 herolist.json（132 英雄），其中 **77 个完成人工威胁标注（9 类标签，覆盖率 58.3%）**，威胁标签集中在 hero_profiles.py 可维护；未知英雄仍明确披露，不猜测。
