...
> **[在线体验 H5 演示](https://olivia-happy.github.io/moba-build-agent/)** · 界面预览与自动演示 GIF 见下方 §3。
>
> **想知道建议是怎么算出来的？** → [`GAME_BREAKDOWN.md`](GAME_BREAKDOWN.md)：装备标签体系（26 类）、约束模型（6 格 / 鞋类唯一组 / 复活甲限次）、英雄威胁标签（9 类 / 77 个英雄）、阵容威胁映射规则（5 条），以及三个对局场景的完整推导复盘。
...
- **决策**：阵容威胁聚合 → 需求排序 → 装备覆盖，每条建议带 `evidence`、价格、组件路径、`why_now`；并叠加**我方定位修正**（脆皮优先保命 / 后排补物防 / 前排提前堆物防）。
- **紧急秒换**：被切/要打关键团时，在金币、格子、鞋类唯一组与复活甲 2 次约束下给可执行方案。
- **可追溯**：装备知识库接入官方 item.json（121 件，含来源 URL 与内容哈希校验），英雄画像接入官方 herolist.json（132 英雄，其中 77 个完成威胁标注）。数值体系的逐层摊开见 [`GAME_BREAKDOWN.md`](GAME_BREAKDOWN.md)。
...
1. **补标高危英雄**：名录 132 个中已标注 77 个（58.3%），优先补刺客 / 强控 / 高回复类——演示场景里兰陵王、赵云、王昭君尚无画像，直接影响推荐强度（见 `GAME_BREAKDOWN.md` §6）。
2. **修正主动装备冷却解析**：当前 `_cooldown()` 取文本中第一个"`N秒`"，血魔之怒 / 纯净苍穹被误解析为 4 / 3 秒（实际 CD 为 75 秒）。
3. **把已标注但未接入的 4 类英雄标签接入映射**：`mobility / movement_speed / shield / true_damage` 目前只记录、未参与需求计算。
4. 真机截图坐标校准（当前为固定布局版本，需可视化拖框）。
